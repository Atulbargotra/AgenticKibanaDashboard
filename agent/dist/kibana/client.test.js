...
describe("kibanaRequest", () => {
    ...
    it("throws on non-ok response", async () => {
        ...
        await expect(kibanaRequest("/api/test", { method: "GET" })).rejects.toThrow("HTTP GET http://localhost:5601/api/test failed: 404 Not Found");
        vi.unstubAllGlobals();
    });
    it("returns parsed JSON on success", async () => {
        ...
        const result = await kibanaRequest("/api/test", { method: "GET" });
    ...
    it("sends correct headers including kbn-xsrf", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith("http://localhost:5601/api/test", expect.objectContaining({
            method: "POST",
            headers: {
