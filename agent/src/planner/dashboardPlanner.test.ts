...
import { z } from "zod";
import type { PatternFinding } from "../types/domain.js";
...
import {
  ...
  fallbackOrSkip,
  planDashboard,
} from "./dashboardPlanner.js";
import { env } from "../config/env.js";

function makeFinding(overrides: Partial<PatternFinding> = {}): PatternFinding {
  return {
...
describe("extractJson", () => {
  ...
  it("throws when no JSON found", () => {
    expect(() => extractJson("no json here")).toThrow(
      "Planner response did not contain JSON"
    );
  });
...
describe("conciseReason", () => {
  ...
  it("replaces 429 errors with a friendly message", () => {
    expect(conciseReason("HTTP 429 Too Many Requests")).toBe(
      "AI planner was rate limited by the upstream model provider"
    );
  });
